...
const ProntoVistaPrevGal: React.FC<ProntoVistaPrevGalProps> = ({ imagenesLista, discosColor = "#000", maxAltura = 32, iteracionTiempo = 3400, navegador = true }) => {
    ...
    const loadingImage = useCallback(({ color = seleccionColor, alto = 24 }) => {
        return React.createElement('div', { style: { color: color, position: 'absolute', inset: '0', display: 'flex', alignContent: 'center', justifyContent: 'center', opacity: loadedImages[currentGalleryIndex] ? 1 : 1, background: 'linear-gradient(0deg, rgba(187,187,187,1) 0%, rgba(245,245,245,1) 100%)', transition: 'all 300ms ease-in-out' } },
    ...
    }, [seleccionColor, loadedImages, currentGalleryIndex])
        ...
        return imagenesLista.map((item, index) => {
                ...
                return React.createElement("div", { key: index, ref: (el) => { imageRefs.current[index] = el as HTMLDivElement | null }, style: imageBlockStyleA },
                    React.createElement("div", { style: imageBlockStyleB },
                        !loadedImages[index] && (loadingImage({ color: seleccionColor, alto: 15 })),
    ...
    }, [imagenesLista, loadingImage, loadedImages, seleccionColor]);
