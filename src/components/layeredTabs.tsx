...
const UsePantallaTamagnos = () => {
  ...
  const getZIndex = (index: number, currentTab: number, topZIndex: number) =>
    //if (currentMouseOverTag === index) return topZIndex + 2;
    // if (currentTab === index) return topZIndex + 1;
    topZIndex - Math.abs(index - currentTab); 
  ...
  const shortenedTitles = useMemo(() =>
    tabsTitleList.map((title, index) => {
      ...
      const longShortened = shortenTitle(title as string, tabsTitleLangList[index] ? tabsTitleLangList[index] : 'en', 20);
      return {
        original: title,
        shortened,
        longShortened,
...
*/
  ...
  const tabShoulderBlock = useCallback( (index: number, pos: number) => {
    ...
    const styleB = { transition: 'width 150ms ease-in-out', display: 'inline-block', background: tabBgColor(index), padding: '0', margin: '0', position: 'relative', boxSizing: 'border-box', height: `100%`, aspectRatio: '1056 / 1486', clipPath: tabShoulder };
