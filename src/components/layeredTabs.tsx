...
const LayeredTabs: LayeredTabsComponent = ({ fondoBarColor = "FFFFFF", ptgnBarColor = "FFFFFF", fondoColor = "FFFFFF", slcPptgnColor = "FFFFFF", tabBarPosition = 0, maxSize = "xl", children }) => {
...
const UsePantallaTamagnos = () => {
  ...
  const getZIndex = (index: number, currentTab: number, topZIndex: number) => topZIndex - Math.abs(index - currentTab);
        ...
        return React.createElement('div', { style: { borderRadius: '0.38rem', overflow: 'hidden', display: 'block', padding: '0', margin: xlScreen || lgScreen ? '3.5rem 0 0 0' : mdScreen ? '2.5rem 0 0 0' : smScreen ? '2rem 0 0 0' : '2rem 0 0 0', position: 'relative', boxSizing: 'border-box', width: `100%`, height: `auto`, background: '#' + fondoColor} },
          ...
          React.createElement('div', { style: { display: 'block', padding: '0', margin: '0', position: 'relative', boxSizing: 'border-box', width: `100%`, height: `auto` } },
            ...
            ( tabBarPosition === 2 || tabBarPosition === 3 ) && React.createElement('div', { style: { zIndex: 10, height: dinamicSize(13) + 'rem', overflowY: 'scroll', maskImage: 'linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(0,0,0,1) 9%, rgba(0,0,0,1) 91%, rgba(0,0,0,0) 100%)', display: 'block', background: '#' + fondoColor, padding: dinamicSize(1.3)+ 'rem ' + dinamicSize(1.5) + 'rem', margin: '0', position: 'relative', boxSizing: 'border-box', width: `100%` } },
              React.createElement('p', { style: { transition: 'all 150ms ease-in-out', display: 'block', position: 'relative', boxSizing: 'border-box', hyphens: 'auto', textAlign: 'justify', textIndent: dinamicSize(1) + 'rem', fontSize: dinamicSize(1.125) + 'rem', fontWeight: 400, color: 'rgba(51,65,85,0.6)', lineHeight: 1.625 } }, tabsContentList[currentTab] ) ),
            ...
            ( tabBarPosition === 0 || tabBarPosition === 1 ) && React.createElement('div', { style: { transition: 'all 150ms ease-in-out', zIndex: 10, height: dinamicSize(13) + 'rem', overflowY: 'scroll', maskImage: 'linear-gradient(to bottom, rgba(0,0,0,0) 0%, rgba(0,0,0,1) 9%, rgba(0,0,0,1) 91%, rgba(0,0,0,0) 100%)', display: 'block', background: '#' + fondoColor, padding: dinamicSize(1.3)+ 'rem ' + dinamicSize(1.5) + 'rem', margin: '0', position: 'relative', boxSizing: 'border-box', width: `100%` } },
              React.createElement('p', { lang: 'la', style: { transition: 'all 150ms ease-in-out', display: 'block', position: 'relative', boxSizing: 'border-box', hyphens: 'auto', textAlign: 'justify', textIndent: dinamicSize(1) + 'rem', fontSize: dinamicSize(1.125) + 'rem', fontWeight: 400, color: 'rgba(51,65,85,0.6)', lineHeight: 1.625 } }, tabsContentList[currentTab] ) ),
