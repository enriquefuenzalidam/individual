...
const ProntoVistaFull: React.FC<ProntoVistaFullProps> = ({ imagenesLista, indice, seleccColor }) => {
    ...
    return React.createElement('div', {ref: mainRef, style: { position: 'relative', height: '100%'} },
                ...
                React.createElement('section', { onClick: () => setMostrarOcultarLista(!mostrarOcultarLista), style: { display: 'block', boxSizing: 'border-box', position: 'absolute', left: '50%', transform: 'translateX(-50%)',  bottom: mostrarOcultarLista ? tnScreen ? '6rem' : smScreen ? '7rem' : mdScreen ? '8rem' : lgScreen ? '9rem' : xlScreen ? '10rem' : '10rem' : '0', width: 'auto', height: 'auto', margin:  tnScreen ? '0.62rem auto' : smScreen ? '0.76rem auto' : mdScreen ? '0.85rem auto' : lgScreen ? '0.91rem auto' : xlScreen ? '1rem auto' : '1rem auto', padding: tnScreen ? '0.29rem 1.62rem 0.15rem 1.62rem' : smScreen ? '0.29rem 1.76rem 0.24rem 1.76rem' : mdScreen ? '0.29rem 1.85rem 0.24rem 1.85rem' : lgScreen ? '0.38rem 1.95rem 0.24rem 1.95rem' : xlScreen ? '0.29rem 2.05rem 0.24rem 2.05rem' : '0.29rem 2.05rem 0.24rem 2.05rem', color: 'rgba(255,255,255,0.38)', fontFamily: 'Sans-serif', fontWeight: '200', fontSize: tnScreen ? '1rem' : smScreen ? '1.09rem' : mdScreen ? '1.15rem' : lgScreen ? '1.24rem' : xlScreen ? '1.38rem' : '1.38rem', lineHeight: 'auto', background: 'rgba(0,0,0,0.38)', borderRadius: '0.38rem', transition: 'all 300ms ease-in-out', textTransform: 'uppercase', letterSpacing: 'normal', cursor: 'pointer' } },
                            '◼︎ ◼︎ ◼︎' ) 
