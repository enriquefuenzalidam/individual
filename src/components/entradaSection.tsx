...
interface EntradaSeccProps {
  articuloTitulo: string;
  href: string;
  articuloTexto: (React.ReactNode | string)[]; }


const EntradaSeccion = ( {articuloTitulo, articuloTexto, href}: EntradaSeccProps ) => {

  const { screenReady, dynSzGttr } = useIndividualStyling();

  if (!screenReady) return null;

    return React.createElement('section', { key: href, style: { display: 'block', padding: '0', margin: '0', boxSizing: 'border-box', position: 'relative', transition: 'opacity 400ms ease-in-out', fontFamily: '"Pangea Trial", sans-serif', letterSpacing: '-0.008rem' } },
